...
class TodoStore:
    ...
    def all(self):
        return [self._restore(uuid) for uuid, value in self.db.items() if not value["deleted"] ]
    ...
    def sync(self):
        ...
        removed = [value["uuid"] for value in self.db.values() if value["uuid"] not in fetched_uuids]
